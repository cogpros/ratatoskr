...
```
agent ──▶ ratatoskr (fetch) ──▶ BIFROST (gate) ──▶ context
              │                    │
        platform-aware        tier 1: regex pre-scan
        routing per URL       tier 2: Red Viper checks
                              tier 3: output post-scan
```
...
- Scans fetched content for prompt injection in three tiers before your agent sees a single character
- Returns a structured summary by default (smallest injection surface), full cleaned text with `--raw`, or machine-readable JSON with `--json`
...
```bash
python3 bifrost.py "https://example.com"            # summary mode (default)
python3 bifrost.py "https://example.com" --raw      # full cleaned text
python3 bifrost.py "https://example.com" --json     # scan results + content as JSON
python3 bifrost.py "https://x.com/user/status/123"  # X route: bird → Jina
```
...
|---|---|---|
| `bifrost.py <url>` | Structured summary | Smallest |
| `bifrost.py <url> --raw` | Full cleaned text | Larger — conscious choice for research/docs |
| `bifrost.py <url> --json` | JSON: content, scan verdicts, warnings | Machine-readable |
...
| `[Summary unavailable...]` prefix | No LLM key | Normal. Content below it passed the gate; use `--raw` if you want everything |
...
├── README.md             # This file
├── LICENSE.txt           # MIT
├── bifrost.py            # Pipeline entry: gate, scan tiers, output modes
